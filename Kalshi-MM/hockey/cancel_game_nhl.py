#!/usr/bin/env python3
"""Cancel all resting Kalshi orders for a specific NHL game at puck drop.

Usage:
    python cancel_game_nhl.py BOSCOL

The game code (e.g. BOSCOL = Boston @ Colorado) is matched as a substring
against each open order's ticker.  Orders for other games are not touched.
"""
...
def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: python cancel_game_nhl.py <GAME_CODE>  e.g. BOSCOL")
        sys.exit(1)

    game_code  = sys.argv[1].upper()
    game_code2 = game_code[3:] + game_code[:3]   # reversed away/home

    os.makedirs(LOGS_DIR, exist_ok=True)
    log_path    = os.path.join(LOGS_DIR, f"{_today}_cancel_at_gametime_nhl.log")
    ...
    with open(log_path, "a", encoding="utf-8") as lf:
        ...
        log(f"PUCK-DROP  game={game_code} — fetching resting orders …")
